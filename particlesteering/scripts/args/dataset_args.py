...
import argparse
import yaml
from typing import Optional, List, Type, Union, get_args, get_origin


def _is_list_annotation(field_type: Type) -> bool:
    """True for List[...] and Optional[List[...]] (Union[List[...], None])."""
    origin = get_origin(field_type)
    if origin is list:
        return True
    if origin is Union:
        return any(_is_list_annotation(a) for a in get_args(field_type) if a is not type(None))
    return False
...
class DatasetArgs:
    ...
    def __init__(
        ...
        for field_name, field_def in fields.items():
            ...
            if _is_list_annotation(field_type):
                parser.add_argument(
                    f'--{field_name}',
                    type=str,
                    nargs='+',
        ...
        # CLI overrides for list/int fields arrive as strings; yaml loads native types.
        if self.steering_factors is not None:
            self.steering_factors = [float(x) for x in self.steering_factors]
        if self.steering_layers is not None and (
            not self.steering_layers or isinstance(self.steering_layers[0], str)
        ):
            self.steering_layers = [int(x) for x in self.steering_layers]
        if self.steering_num_of_examples is not None:
            self.steering_num_of_examples = int(self.steering_num_of_examples)
