...
class ModelParams:
    ...
    randomize_train_steering_factors: Optional[bool] = False
    # RePS-style (default): infer strength = factor * w. SDL/LsReFT alternative: factor * max_act * w.
    use_max_act_steering_calibration: Optional[bool] = False
    # SDL/LsReFT-style: keep ||w||=1 during train (repulsion on sphere); pair with max_act at infer.
    use_unit_norm_steering_vectors: Optional[bool] = False
    # Train DPO with AdditionIntervention: Δh = mag * max_act * w (online prompt peak), same as SDL infer.
    # Default false keeps PreferenceVectorIntervention + (α+bias)*v. A/B vs legacy without removing it.
    use_sdl_training_intervention: Optional[bool] = False
    ...
    # Multi-particle HyperSteer (HyperSteerParticles)
    num_particles: Optional[int] = 4
    # LsReFTParticles: per-row proj init (see lsreft_particles.init_particle_proj_weights)
    particle_weight_init: Optional[str] = None  # orthogonal | random | legacy
    particle_init_scale_min: Optional[float] = None
    particle_init_scale_max: Optional[float] = None
    # LsReFTParticles: validity aggregation (see particle_objectives.py)
    particle_validity_mode: Optional[str] = None
    particle_validity_std_weight: Optional[float] = None
    particle_validity_softmax_temp: Optional[float] = None
    # LsReFTParticles: repulsion (legacy names still accepted)
    repulsion_type: Optional[str] = None
    repulsion_formulation: Optional[str] = None
    repulsion_weight: Optional[float] = None
    repulsion_lambda: Optional[float] = None
    repulsion_bandwidth: Optional[float] = None
    repulsion_reference_r: Optional[float] = None
    repulsion_threshold_tau: Optional[float] = None
    repulsion_validity_scale: Optional[float] = None
    repulsion_warmup_steps: Optional[int] = None
    particle_diversity_weight: Optional[float] = 0.0
    particle_rbf_bandwidth: Optional[float] = 1.0
    lm_loss_aggregation: Optional[str] = "mean"
    train_particles_mode: Optional[str] = "all"
    use_amortized_svgd_train: Optional[bool] = False
    steer_all_particles: Optional[bool] = True
    steering_particle_policy: Optional[str] = "all"
    steering_particle_index: Optional[int] = None
    # LsReFTParticles: reset RNG per (concept, input, factor) before each particle generate
    steering_shared_rng_per_row: Optional[bool] = False
    # HyperSteerSVGD (amortized Stein variational gradient descent)
    svgd_stepsize: Optional[float] = 1e-3
    svgd_bandwidth: Optional[float] = -1.0
    svgd_lm_aux_weight: Optional[float] = 0.0
...
class TrainingArgs:
    def __init__(
        ...
        hierarchical_params = [
            ...
            'reference_free', 'label_smoothing', 'steering_factors', 'randomize_train_steering_factors',
            'use_max_act_steering_calibration', 'use_unit_norm_steering_vectors',
            'use_sdl_training_intervention',
            'negative_only', 'simpo_scaler', 
            ...
            'num_particles',
            'particle_weight_init', 'particle_init_scale_min', 'particle_init_scale_max',
            'particle_validity_mode', 'particle_validity_std_weight', 'particle_validity_softmax_temp',
            'repulsion_type', 'repulsion_formulation', 'repulsion_weight', 'repulsion_lambda',
            'repulsion_bandwidth', 'repulsion_reference_r', 'repulsion_threshold_tau',
            'repulsion_validity_scale', 'repulsion_warmup_steps',
            'particle_diversity_weight', 'particle_rbf_bandwidth',
            'lm_loss_aggregation', 'train_particles_mode', 'use_amortized_svgd_train',
            'svgd_stepsize', 'svgd_bandwidth', 'svgd_lm_aux_weight',
            'steer_all_particles', 'steering_particle_policy', 'steering_particle_index',
            'steering_shared_rng_per_row',
        ]
        ...
        for model_name in self.models_list:
            ...
            model_param_fields = set(ModelParams.__dataclass_fields__.keys())
            if model_name in self.model_params:
                ...
                for param_name, param_value in model_config.items():
                    if param_name in model_param_fields:
                        setattr(params, param_name, param_value)
                    else:
                        print(
                            f"Warning: unknown key '{param_name}' under models.{model_name} "
                            "(ignored)"
                        )
        ...
        for param_str in args.model_param:
            ...
            model_param_fields = set(ModelParams.__dataclass_fields__.keys())
            if param_name not in model_param_fields:
    ...
    def _infer_type(param_name: str):
        bool_params = ['use_bf16', 'exclude_bos', 'binarize_dataset', 'train_on_negative', 
            ...
            'randomize_train_steering_factors', 'use_max_act_steering_calibration',
            'use_unit_norm_steering_vectors', 'use_sdl_training_intervention',
            'use_amortized_svgd_train', 'steer_all_particles', 'steering_shared_rng_per_row']
        int_params = ['layer', 'batch_size', 'n_epochs', 'topk', 'seed', 'low_rank_dimension', 
                      ...
                      'num_particles', 'steering_particle_index', 'repulsion_warmup_steps']
        float_params = [
            'lr', 'coeff_l1_loss_null', 'coeff_l1_loss', 'coeff_l2_loss', 'coeff_norm_loss', 
            ...
            'particle_validity_std_weight', 'particle_validity_softmax_temp',
            'repulsion_weight', 'repulsion_lambda', 'repulsion_bandwidth',
            'repulsion_reference_r', 'repulsion_threshold_tau', 'repulsion_validity_scale',
            'particle_diversity_weight', 'particle_rbf_bandwidth',
            'particle_init_scale_min', 'particle_init_scale_max',
            'svgd_stepsize', 'svgd_bandwidth', 'svgd_lm_aux_weight']
        str_params = [
            ...
            'particle_weight_init',
            'particle_validity_mode', 'repulsion_type', 'repulsion_formulation',
            'lm_loss_aggregation', 'train_particles_mode', 'steering_particle_policy']
