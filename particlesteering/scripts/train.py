...
#     torchrun --nproc_per_node=NUM_GPUS particlesteering/scripts/train.py --config configs/train.yaml
import os
import argparse
import yaml
import json
...
from particlesteering.models.sae import save_pruned_sae
...
def main():
    ...
    for concept_id, concept_df in my_df_list:
        ...
        for model_name in sorted(args.models.keys()):
            ...
            benchmark_model = getattr(particlesteering, model_name)(
    ...
    if rank == 0:
        ...
        # Save other config (include particle objective for audit / inference provenance)
        config = {
            "model_name": args.model_name,
            "layer": args.layer,
            "component": args.component,
        }
        from particlesteering.models.particle_objectives import particle_config_from_training_args

        if "ParticleSteering" in args.models.keys():
            lp = args.models["ParticleSteering"]
            config["ParticleSteering"] = {
                    **particle_config_from_training_args(lp),
                    "particle_weight_init": getattr(lp, "particle_weight_init", None) or "orthogonal",
                    "particle_init_scale_min": float(
                        getattr(lp, "particle_init_scale_min", None) or 0.01
                    ),
                    "particle_init_scale_max": float(
                        getattr(lp, "particle_init_scale_max", None) or 0.03
                    ),
                    "loss_type": getattr(lp, "loss_type", None),
                    "use_dpo_loss": getattr(args, "use_dpo_loss", False),
                    "use_max_act_steering_calibration": bool(
                        getattr(lp, "use_max_act_steering_calibration", False)
                    ),
                    "use_unit_norm_steering_vectors": bool(
                        getattr(lp, "use_unit_norm_steering_vectors", False)
                    ),
                    "use_sdl_training_intervention": bool(
                        getattr(lp, "use_sdl_training_intervention", False)
                    ),
            }
