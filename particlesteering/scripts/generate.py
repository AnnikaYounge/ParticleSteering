...
#    python particlesteering/scripts/generate.py --config configs/generate.yaml
...
from particlesteering.utils.dataset import DatasetFactory
...
def load_concepts(dump_dir):
    ...
    if ".txt" in dump_dir:
        ...
        # One column: concept text only. Optional second column: Neuronpedia ref (SAE runs only).
        sae_concepts = []
        refs = []
        with open(dump_dir, 'r') as file:
            ...
            for row in reader:
                if not row or not str(row[0]).strip():
                    continue
                sae_concepts.append(row[0].strip())
                refs.append(row[1].strip() if len(row) > 1 and str(row[1]).strip() else "")
        return sae_concepts, refs
...
def generate_training(args, generate_args):
    ...
    # Load lm and tokenizer (for shared negative examples; set generate.model_name in YAML).
    model_name = _resolve_generate_base_model_name(args, all_refs)
    logger.warning(f"Generate base model for negative examples: {model_name}")
