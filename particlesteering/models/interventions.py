...
class AdditionIntervention(
    ...
    def forward(self, base, source=None, subspaces=None):
        # use subspaces["idx"] to select the correct weight vector
        w = self.proj.weight[subspaces["idx"]]
        steering_vec = subspaces["max_act"].unsqueeze(dim=-1) * \
            subspaces["mag"].unsqueeze(dim=-1) * w
        output = base + steering_vec.to(base.dtype).unsqueeze(dim=1)
        # SDL train + latent_corr: functional detector s_t = ReLU(h_t · w) (no bias).
        if w.dim() == 1:
            latent = torch.relu(torch.einsum("bsh,h->bs", base, w))
        else:
            latent = torch.relu(torch.einsum("bsh,bh->bs", base, w))
        return InterventionOutput(
            output=output.to(base.dtype),
            latent=[latent],
        )
...
def _proj_row_index(subspaces: Optional[dict]) -> int:
    """Particle / concept row in proj.weight from subspaces['idx'] (scalar)."""
    row = 0
    if subspaces is not None and "idx" in subspaces:
        row = subspaces["idx"]
...
def _proj_rows_for_subspaces(
    proj_weight: torch.Tensor,
    base: torch.Tensor,
    subspaces: dict,
) -> list:
    """
    Build one weight row per base batch row.

    With pyvene unit_locations, base.shape[0] is not the DPO minibatch size.
    Pass subspaces["idx"] (scalar or 0-d tensor), not subspaces["subspaces"].
    """
    if "subspaces" not in subspaces:
        row = _proj_row_index(subspaces)
        return [proj_weight[row] for _ in range(base.shape[0])]

    sub = subspaces["subspaces"]
    if isinstance(sub, torch.Tensor):
        if sub.shape[0] != base.shape[0]:
            raise ValueError(
                f"subspaces tensor length ({sub.shape[0]}) != base batch dim "
                f"({base.shape[0]}). For DPO / unit_locations use subspaces['idx'], "
                "not subspaces['subspaces']."
            )
        return [proj_weight[int(sub[i].item())] for i in range(base.shape[0])]

    rows = []
    for subspace in sub:
        rows.append(proj_weight[subspace])
    if len(rows) != base.shape[0]:
        raise ValueError(
            f"len(subspaces['subspaces']) ({len(rows)}) != base batch dim ({base.shape[0]}). "
            "For DPO / unit_locations use subspaces['idx']."
        )
    return rows
...
class ConceptVectorIntervention(
    ...
    def forward(
        ...
        v = _proj_rows_for_subspaces(self.proj.weight, base, subspaces)
        v = torch.stack(v, dim=0).unsqueeze(dim=-1) # bs, h, 1
        latent = torch.relu(torch.bmm(base, v)).squeeze(dim=-1) # bs, s, 1
        steering_vec = v.permute(0, 2, 1) # bs, 1, h
...
class PreferenceVectorIntervention(
    ...
    def forward(
        ...
        row = _proj_row_index(subspaces)
        bias_row = self.proj.bias[row]
        v = _proj_rows_for_subspaces(self.proj.weight, base, subspaces)
        v = torch.stack(v, dim=0).unsqueeze(dim=-1) # bs, h, 1
        v_norm = torch.norm(v, dim=1, keepdim=True) # bs, 1, 1
        # One rank-1 direction per forward (row); do not add full proj.bias [N*R] (LsReFT/TopK path).
        latent = torch.relu((torch.bmm(base, v) + bias_row).squeeze(dim=-1)) # bs, s, 1
        steering_vec = v.permute(0, 2, 1) # bs, 1, h
        steering_vec = self.dropout(steering_vec)
        
        if "steering_factor" in subspaces:
            steering_factor = subspaces["steering_factor"].unsqueeze(dim=-1).unsqueeze(dim=-1) # bs, 1, 1
            if steering_factor.shape[0] != base.shape[0]:
                raise ValueError(
                    f"steering_factor batch ({steering_factor.shape[0]}) != base batch dim "
                    f"({base.shape[0]}). Check DPO minibatch stacking / unit_locations."
                )
            ...
            combined_steering_factor = null_it_out_steering_factor + (steering_factor + bias_row*nonzero_mask) # bs, s, 1
