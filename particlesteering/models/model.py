...
class Model(BaseModel):
    ...
    def load(self, dump_dir=None, **kwargs):
        ...
        if priority_mode == "mem_priority":
            ...
            num_particles = int(kwargs.get("num_particles", 1))
            if num_particles > 1:
                sl = slice(concept_id * num_particles, (concept_id + 1) * num_particles)
                weight_rank = weight[sl]
                bias_rank = bias[sl]
                kwargs = dict(kwargs)
                kwargs["low_rank_dimension"] = num_particles
            else:
                weight_rank = weight[concept_id].unsqueeze(0)
                bias_rank = bias[concept_id].unsqueeze(0)
            # load only 1 rank to prevent OOM, and faster inference
            self.make_model(**kwargs)
            self.ax.proj.weight.data = weight_rank.to(self.device)
            self.ax.proj.bias.data = bias_rank.to(self.device)
